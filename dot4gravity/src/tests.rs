...
fn a_stone_dropped_should_increase_score() {
    ...
    assert_eq!(state.get_player_score(&ALICE), Score::default() + NB_POINT_STONE);
    assert_eq!(state.get_player_score(&BOB), Score::default());

    state = Game::drop_stone(state, BOB, Side::North, 0).unwrap();
    assert_eq!(state.get_player_score(&ALICE), Score::default() + NB_POINT_STONE);
    assert_eq!(state.get_player_score(&BOB), Score::default() + NB_POINT_STONE);
...
fn destroy_its_own_stone_should_not_increase_player_score() {
    ...
    assert_eq!(state.get_player_score(&ALICE), Score::default() + NB_POINT_STONE * 2);
}
...
fn should_play_a_game() {
    ...
    // Alice played 5 stones and destroyed 2 Bob stones
    let alice_score = Score::default() + NB_POINT_STONE * 5 + 2 * NB_POINT_ENEMY_STONE_DESTROYED;
    // Bob played 5 stones and didn't destroy any Alice stone
    let bob_score = Score::default() + NB_POINT_STONE * 5;
